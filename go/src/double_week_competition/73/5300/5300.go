...
func getAncestors(n int, edges [][]int) [][]int {
	...
	var counter = make([]map[int]int, n)
	for i := range counter {
	...
	for i := 0; i < n; i++ {
		for k := range counter[i] {
			j := LowerBound(ans[i], k)
			ans[i] = append(ans[i], 0)
			copy(ans[i][j+1:], ans[i][j:])
			ans[i][j] = k
		}
	}
	fmt.Println(ans)
