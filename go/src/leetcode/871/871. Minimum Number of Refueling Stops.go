...
思路: 路上的不是加油站，而是一桶桶的油，每次经过的时候，就把油带上，当油不够的时候我们就取身上最大的那桶油加上，
		这样如果身上没油了，那么就到不了了
*/
func minRefuelStops(target int, startFuel int, stations [][]int) int {
	...
	var x int
	var i, pos = 0, startFuel

	for pos < target {
		for i < ns && stations[i][0] <= pos {
...
func main() {
	fmt.Println(minRefuelStops(100, 10, [][]int{{10, 60}, {20, 30}, {30, 30}}))
